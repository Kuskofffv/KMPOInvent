...
import 'package:kmpo_invent/utils/calendar/provider_image.dart';
...
typedef EventCellBuilder = Function(BuildContext context,
    NeatCleanCalendarEvent event, String start, String end);
...
class Calendar extends StatefulWidget {
  ...
  const Calendar(
      ...
      this.titleDateFormat = 'MMMM yyyy',
...
class _CalendarState extends State<Calendar> {
    ...
    if (widget.locale != null) {
      initializeDateFormatting(widget.locale, null).then((_) {
        if (mounted) {
          setState(() {
            final monthFormat =
                DateFormat(widget.titleDateFormat, widget.locale)
  ...
  void scrollToTop() {
    ...
    _scrollController.animateTo(
      0, // Die Scroll-Position (0.0 ist ganz oben)
  ...
  int findClosestIndex(DateTime currentDate) {
    ...
    for (int i = 0; i < itemList.length; i++) {
      if (itemList[i] is DateTime) {
        final DateTime eventDate = itemList[i] as DateTime;

        // Berechne die Differenz in Tagen zwischen dem Event-Datum und dem aktuellen Datum
        final int difference = eventDate.difference(currentDate).inDays.abs();
  ...
  void _updateEventsMap() {
    ...
    if (widget.eventsList != null &&
        ...
        eventsMap != null &&
        eventsMap!.isEmpty) {
      for (final event in widget.eventsList!) {
        ...
        if (range == 0) {
          final List<NeatCleanCalendarEvent> dateList = eventsMap![DateTime(
          ...
          for (var i = 0; i <= range; i++) {
            ...
            final NeatCleanCalendarEvent newEvent = NeatCleanCalendarEvent(
                event.summary,
  ...
  Widget get nameAndIconRow {
    ...
    if (!widget.hideTodayIcon) {
      todayIcon =
          Text(widget.todayButtonText, style: widget.displayMonthTextStyle);
    } else {
      todayIcon = Container();
    }
        ...
        onPressed: () {
          showDatePicker(
                  builder: (context, child) {
                    // Define Light Theme
                    final ThemeData lightTheme = widget.datePickerLightTheme ??
                    ...
                    // Define Dark Theme
                    final ThemeData darkTheme = widget.datePickerDarkTheme ??
    ...
    return Stack(
      ...
      children: [
        Row(
          ...
          children: [
            if (forceEventListView) Container() else leftArrow,
            if (widget.showEventListViewIcon)
              PlatformIconButton(
                onPressed: () {
                  setState(() {
            ...
            if (forceEventListView) Container() else jumpDateIcon,
            if (forceEventListView) Container() else rightArrow,
          ],
        ),
  ...
  Widget get eventlistView {
    ...
    final List<NeatCleanCalendarEvent>? _listEvents =
  ...
  void resetToToday() {
    ...
    if (widget.eventListBuilder != null) {
      final int index = findClosestIndex(DateTime.now());
      if (index != -1) {
        scrollToIndex(index);
      } else {
        scrollToTop();
  ...
  void nextMonth(bool launchCallback) {
    final DateTime _newDate = Utils.nextMonth(_selectedDate);
    ...
    setState(() {
      ...
      final firstDateOfNewMonth = Utils.firstDayOfMonth(_selectedDate);
      final lastDateOfNewMonth = Utils.lastDayOfMonth(_selectedDate);
      updateSelectedRange(firstDateOfNewMonth, lastDateOfNewMonth);
      selectedMonthsDays = _daysInMonth(_selectedDate);
      final monthFormat = DateFormat(widget.titleDateFormat, widget.locale)
          .format(_selectedDate);
      displayMonth =
          '${monthFormat[0].toUpperCase()}${monthFormat.substring(1)}';
      _selectedEvents = eventsMap?[DateTime(
  ...
  void previousMonth(bool launchCallback) {
    final DateTime _newDate = Utils.previousMonth(_selectedDate);
    // Parameter "launchCallback" is there to avoid triggering the callback twice.
    if (launchCallback) {
      _launchDateSelectionCallback(_newDate);
    }
  ...
  void nextWeek() {
    final DateTime _newDate = Utils.nextWeek(_selectedDate);
    _launchDateSelectionCallback(_newDate);
    setState(() {
      _selectedDate = _newDate;
      final firstDayOfCurrentWeek = _firstDayOfWeek(_selectedDate);
      final lastDayOfCurrentWeek = _lastDayOfWeek(_selectedDate);
  ...
  void previousWeek() {
    final DateTime _newDate = Utils.previousWeek(_selectedDate);
    _launchDateSelectionCallback(_newDate);
    setState(() {
      _selectedDate = _newDate;
      final firstDayOfCurrentWeek = _firstDayOfWeek(_selectedDate);
  ...
  void updateSelectedRange(DateTime start, DateTime end) {
    final Range _rangeSelected = Range(start, end);
  ...
  void handleSelectedDateAndUserCallback(DateTime day) {
    ...
    final firstDayOfCurrentWeek = _firstDayOfWeek(day);
    final lastDayOfCurrentWeek = _lastDayOfWeek(day);
  ...
  dynamic _lastDayOfWeek(DateTime date) {
...
class ExpansionCrossFade extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedCrossFade(
      ...
      firstCurve: const Interval(0, 1, curve: Curves.fastOutSlowIn),
